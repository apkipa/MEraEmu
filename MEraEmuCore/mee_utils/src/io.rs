...
use binrw::{BinReaderExt, BinResult, BinWriterExt, Endian, binrw, parser, writer};
...
pub trait CSharpBinaryReader {
    ...
    fn read_var_i32(&mut self) -> std::io::Result<i32>;
...
pub trait CSharpBinaryWriter {
    ...
    fn write_var_i32(&mut self, val: i32) -> std::io::Result<()>;
...
impl<T: std::io::Read> CSharpBinaryReader for T {
    ...
    fn read_utf16_string(&mut self) -> std::io::Result<String> {
        let bytes_len = self.read_var_i32()?;
        if bytes_len < 0 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
...
impl<T: std::io::Write> CSharpBinaryWriter for T {
    ...
    fn write_utf16_string(&mut self, val: &str) -> std::io::Result<()> {
        ...
        self.write_var_i32(bytes_cnt.try_into().map_err(|_| {
...
pub fn write_var_i32(val: i32) -> BinResult<()> {
    Ok(writer.write_var_i32(val)?)
}

// TODO: Deduplicate string reading/writing code caused by binrw discrepancy
//       (requiring Seek + endian to be passed in)
...
pub fn write_utf16_string<W>(val: &str, writer: &mut W, endian: Endian) -> BinResult<()>
where
    W: Write + Seek,
{
    let bytes_cnt = val.encode_utf16().count() * 2;
...
pub fn brw_write_utf16_string(val: &String) -> BinResult<()> {
    write_utf16_string(val, writer, endian)
}
