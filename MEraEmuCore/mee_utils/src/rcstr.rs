...
impl ArcStr {
    ...
    pub unsafe fn as_rc(&self) -> &RcStr {
        unsafe { &*(self as *const Self as *const RcStr) }
    }
    ...
    pub unsafe fn as_rc_mut(&mut self) -> &mut RcStr {
        unsafe { &mut *(self as *mut Self as *mut RcStr) }
    }
}
...
macro_rules! format {
    ($($toks:tt)*) => {
        $crate::rcstr::BoxedRcStr::from(&std::fmt::format(core::format_args!($($toks)*))[..]).into()
    };
}
...
macro_rules! literal {
    ($text:expr $(,)?) => {
        $crate::rcstr::BoxedRcStr::from($text).into()
    };
}
